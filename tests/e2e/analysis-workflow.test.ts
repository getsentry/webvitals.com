...
import { hasValidPerformanceData, openrouter } from "@/workflows/analysis";
...
describe("DurableAgent analysis workflow (E2E)", () => {
   ...
   */
  it("calls all 3 tools and streams outputs for sentry.io", async () => {
    ...
    const breakdownInput = toolInputChunks.find(
      (c) =>
        (c as { toolName: string }).toolName === "generateAnalysisBreakdown",
    );
