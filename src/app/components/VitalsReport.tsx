    ...
    return loading ? null : (
        <div className="border-black border mt-4 rounded p-4">
            <ul className="list-none grid grid-cols-2">
                ...
                }} tooltip="Measures the time from when a page starts loading to when any part of the page's content is first displayed." /></li>
                <li><Vital name="Largest Contentful Paint" href="/lcp" score={vitals.LCP} thresholds={{
                    good: 2500,
                    needsImprovement: 4000
                }} tooltip="Measures the time it takes for the largest text or image element to render on a webpage." /></li>
                <li><Vital name="Time to First Byte" href="/ttfb" score={vitals.TTFB} thresholds={{
                    good: 800,
                    needsImprovement: 1800
                }} tooltip="Measures the duration from when a page starts loading to when the first byte of content is received from the server."
                /></li>
                <li><Vital name="Cumulative Layout Shift" href="/cls" score={vitals.CLS} thresholds={{
                    good: 0.1,
                    needsImprovement: 0.25,
                }} formatter={(score: string) => !isNaN(Number(score)) ? Number(score).toFixed(2) : score}
                    tooltip="Measures the total amount of unexpected layout shifts that occur during the entire lifespan of a webpage."
                /></li>
            </ul>
            <ul className="list-none grid grid-cols-2 border-t mt-4 pt-4">
                ...
                }} tooltip="Measures the time from when a user interacts with a page to when the browser renders the visual response to that interaction." /></li>
