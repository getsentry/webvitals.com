      ...
      <div className="p-4 border border-blue-200 bg-blue-50 rounded-lg">
        ...
        <div className="text-sm space-y-1">
          ...
          <p className="text-gray-600">
            ...
            {output.status === "InProgress" && "Scan is currently running..."}
          </p>
        </div>
      </div>
    );
...
function CloudflareSearchOutput({ output }: { output: ScanSearchResponse }) {
  if (!output.success || !output.result?.search?.length) {
    return (
  ...
  const formatted = formatSearchResults(output.result.search);
    ...
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm text-gray-500">
          {output.result.meta.count} of {output.result.meta.total} results
        </span>
      </div>
